...
export async function PUT(req: NextRequest,{ params } : { params: Promise<{ teacherId: string }> }) {
  ...
  try {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/v1/teachers/${teacherId}`, {
      method: 'PUT',
      headers: {
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ teacherId: string }> }) {
    const cookieStore = await cookies();
    const token = cookieStore.get('access_token')?.value;
    const { teacherId } = await params;
